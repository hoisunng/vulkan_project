...
static uint32_t findMemoryTypeIndex(VkPhysicalDevice physicalDevice, uint32_t allowedTypes, VkMemoryPropertyFlags properties)
{
	...
	throw std::runtime_error{ "Memory type index not found!" };
}
...
{
	const auto transferCommandBuffer = beginCommandBuffer(device, transferCommandPool);
...
{
	const auto commandBuffer = beginCommandBuffer(device, commandPool);
